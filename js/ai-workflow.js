...
function markAsDontKnow() { const as=getActiveSet(); if(!as)return; const q=as.questions[as.currentIdx]; if(!q)return; if(q.type!=='single'&&q.type!=='judge')return; var wrongIdx=-1; if(q.options&&Array.isArray(q.options)){for(var i=0;i<q.options.length;i++){if(i!==q.answer){wrongIdx=i;break;}}} if(wrongIdx===-1)return; as.userAnswers[as.currentIdx]=wrongIdx; if(!state.ignoredQuestions)state.ignoredQuestions=[]; var qId=getQuestionId(as.setId,q); if(!state.ignoredQuestions.includes(qId)){state.ignoredQuestions.push(qId);} var ch=getCh(); if(ch&&as._isSet){if(!as._tagSynced)as._tagSynced=[];if(!as._tagSynced[as.currentIdx]){_syncSingleAnswerToTagMeta(ch.id,q,wrongIdx);as._tagSynced[as.currentIdx]=true;}} saveState(); renderQuestion(); updateProgress(); syncAnswerToServer(); }
