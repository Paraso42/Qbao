...
// 模型能力自动检测（静默，无需用户手动设置流式/严格开关）
function _getModelCapabilities() {
  var ac = state.aiConfig || {};
  var model = ac.model || 'ecnu-plus';
  // 确保 providers 已加载
  if (!aiProviders || aiProviders.length === 0) {
    console.warn('[auto-detect] aiProviders not loaded, using defaults');
    return { streaming: false, jsonSchema: true };
  }
  for (var i = 0; i < aiProviders.length; i++) {
    for (var j = 0; j < (aiProviders[i].models || []).length; j++) {
      if (aiProviders[i].models[j].id === model) {
        return aiProviders[i].models[j];
      }
    }
  }
  // 模型不在列表中（如旧版 ecnu-turbo 已删除），回退到 ecnu-plus
  console.warn('[auto-detect] model ' + model + ' not found, using defaults');
  return { streaming: false, jsonSchema: true };
}
function _shouldUseStreaming() {
  return _getModelCapabilities().streaming === true;
}
function _shouldUseStrictFormat() {
  var caps = _getModelCapabilities();
  return caps.jsonSchema === true;
}
...
async function _aiExecuteTask(task) {
  ...
  try {
    ...
    if (!(_shouldUseStreaming() && task.streamSetRef)) {
      createQuizSetForChapter(questions, task.chapterId);
    }
    // Reset new file flag after successful generation
    if (ch && ch._hasNewFilesSinceLastGen) { ch._hasNewFilesSinceLastGen = false; ch._lastGenTime = Date.now(); }
    task.status = 'completed'; task.completedAt = Date.now(); task.questionCount = questions.length;
    if (state.currentChapterId === task.chapterId) { renderSubjectList(); renderTagColumns(); updateQuickActions(); }
  } catch(e) {
    if (e.name === 'AbortError') { task.status = 'failed'; task.error = '用户取消'; }
    else { task.status = 'failed'; task.error = e.message; }
  }
  // Process pool file diagnostics from non-streaming response if not already set by stream
  if (!task.poolFilesTotal && task._poolFilesStatus && Array.isArray(task._poolFilesStatus)) {
    var pfs = task._poolFilesStatus;
    var failed = pfs.filter(function(s) { return !s.extracted || s.empty || s.error; });
    if (failed.length > 0) {
      task.poolFileWarnings = failed.map(function(s) {
        return s.name + ': ' + (s.error || (s.empty ? '内容为空' : '提取失败'));
      });
    }
    var ok = pfs.filter(function(s) { return s.extracted && !s.empty && !s.error; });
  ...
  if (task.status === 'completed') {
    ...
    if (task.strategyCompliance && !task.strategyCompliance.ok) {
      msg += '\n⚠ 策略比例有偏差：期望(' + task.strategyCompliance.expected.error + '/' + task.strategyCompliance.expected.review + '/' + task.strategyCompliance.expected.new + ') 实际(' + task.strategyCompliance.actual.error + '/' + task.strategyCompliance.actual.review + '/' + task.strategyCompliance.actual.new + ')';
    }
    showAiTaskNotification('completed', msg);
  }
  ...
  for (var attempt = 1; attempt <= maxAttempts && !streamCompleted; attempt++) {
    ...
    var res = await fetch(API_BASE + '/ai/generate', {
      ...
      headers: {
        ...
        'x-ai-strict-format': _shouldUseStrictFormat() ? 'true' : 'false',
        'x-ai-stream': 'true'
      },
      body: JSON.stringify({
      ...
      for (var j = 0; j < lines.length; j++) {
        ...
        try {
          ...
          if (evt.done) {
            ...
            // Collect pool file diagnostics from stream response
            if (evt.poolFilesStatus && Array.isArray(evt.poolFilesStatus)) {
              var failedFiles = evt.poolFilesStatus.filter(function(s) { return !s.extracted || s.empty || s.error; });
              if (failedFiles.length > 0) {
                task.poolFileWarnings = failedFiles.map(function(s) {
                  return s.name + ': ' + (s.error || (s.empty ? '内容为空' : '提取失败'));
                });
              }
              var okFiles = evt.poolFilesStatus.filter(function(s) { return s.extracted && !s.empty && !s.error; });
              task.poolFilesUsed = okFiles.length;
              task.poolFilesTotal = evt.poolFilesStatus.length;
            }
            break;
          }
        } catch (e2) {}
