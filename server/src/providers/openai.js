// OpenAI ChatGPT Provider
// Native json_schema + streaming support via https://api.openai.com/v1

const BASE_URL = 'https://api.openai.com/v1';

async function chatCompletions(apiKey, model, messages, options) {
  const key = apiKey || process.env.OPENAI_API_KEY || '';
  const body = {
    model: model || 'gpt-4o',
    messages,
    temperature: options.temperature || 0.7,
    max_tokens: options.max_tokens || 4096
  };

  if (options.response_format) {
    body.response_format = options.response_format;
  }

  const contentLen = (messages[1] && messages[1].content && typeof messages[1].content === 'string')
    ? messages[1].content.length : 0;
  console.log('[openai] posting to', BASE_URL + '/chat/completions',
    'model=' + body.model, 'contentLen=' + contentLen,
    'max_tokens=' + body.max_tokens);

  const controller = new AbortController();
  const timeout = setTimeout(function() { controller.abort(); }, 300000);
  const abortFromCaller = () => controller.abort();
  if (options.signal) {
    if (options.signal.aborted) controller.abort();
    else options.signal.addEventListener('abort', abortFromCaller, { once: true });
  }
  const start = Date.now();

  try {
    const res = await fetch(BASE_URL + '/chat/completions', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + key
      },
      body: JSON.stringify(body),
      signal: controller.signal
    });
    clearTimeout(timeout);
    const ms = Date.now() - start;
    console.log('[openai] response status=' + res.status + ' time=' + ms + 'ms');
    const text = await res.text();
    if (!res.ok) {
      console.log('[openai] error body=' + text.substring(0, 500));
      throw new Error(res.status + ' ' + res.statusText + ': ' + text.substring(0, 200));
    }
    return JSON.parse(text);
  } catch (e) {
    clearTimeout(timeout);
    if (options.signal) options.signal.removeEventListener('abort', abortFromCaller);
    const ms = Date.now() - start;
    if (e.name === 'AbortError') {
      if (options.signal && options.signal.aborted) {
        console.log('[openai] aborted by caller after ' + ms + 'ms');
        throw new Error('已取消');
      }
      console.log('[openai] TIMEOUT after ' + ms + 'ms');
      throw new Error('AI响应超时（超过5分钟），可能是内容过长或网络慢。请减少资料后重试。');
    }
    console.log('[openai] fetch error after ' + ms + 'ms - ' + e.message);
    throw e;
  }
}

async function streamChatCompletions(apiKey, model, messages, options, onEvent, signal) {
  const key = apiKey || process.env.OPENAI_API_KEY || '';
  const body = {
    model: model || 'gpt-4o',
    messages,
    stream: true,
    temperature: options.temperature || 0.7,
    max_tokens: options.max_tokens || 4096
  };

  // OpenAI supports native json_schema with streaming
  if (options.response_format) {
    body.response_format = options.response_format;
  }

  const start = Date.now();
  console.log('[openai] streaming to', BASE_URL + '/chat/completions',
    'model=' + body.model, 'max_tokens=' + body.max_tokens);

  const res = await fetch(BASE_URL + '/chat/completions', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ' + key
    },
    body: JSON.stringify(body),
    signal
  });

  console.log('[openai] stream response status=' + res.status);
  if (!res.ok) {
    const errText = await res.text();
    throw new Error(res.status + ' ' + errText.substring(0, 200));
  }

  let fullContent = '';
  let deltaCount = 0;
  const decoder = new TextDecoder();
  const reader = res.body.getReader();

  while (true) {
    const { done, value } = await reader.read();
    if (done) break;
    const chunk = decoder.decode(value, { stream: true });
    const lines = chunk.split('\n');
    for (const line of lines) {
      if (!line.startsWith('data: ')) continue;
      const data = line.slice(6);
      if (data === '[DONE]') continue;
      try {
        const parsed = JSON.parse(data);
        const delta = parsed.choices?.[0]?.delta?.content;
        if (delta) {
          fullContent += delta;
          deltaCount++;
          if (onEvent) {
            onEvent({ type: 'delta', content: delta, full: fullContent, deltaCount });
          }
        }
      } catch (e) { /* skip */ }
    }
  }

  const elapsed = Date.now() - start;
  console.log('[openai] stream done, totalMs=' + elapsed + ', contentLen=' + fullContent.length);
  return { content: fullContent, elapsed };
}

function supportsJsonSchema(_model) {
  return true;
}

function supportsStreamWithJsonSchema(_model) {
  return true;
}

module.exports = {
  name: 'openai',
  baseURL: BASE_URL,
  chatCompletions,
  streamChatCompletions,
  supportsJsonSchema,
  supportsStreamWithJsonSchema
};
