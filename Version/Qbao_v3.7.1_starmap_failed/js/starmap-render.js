...
export function initStarmapScene(container) {
  ...
  // Controls — LEFT=rotate, MIDDLE=dolly, RIGHT disabled (WASD pan instead)
  ...
  bloomPass = new UnrealBloomPass(
    ...
    isMobile ? 1.0 : 2.5,   // strength — heavy bloom for merged glow clusters
    isMobile ? 0.6 : 1.2,   // radius — wide halo spread
    isMobile ? 0.1 : 0.05,  // threshold — nearly everything blooms
  );
  ...
  sphereGeoDot = new THREE.SphereGeometry(0.12, 4, 4);
...
export function starmapRenderNodes(data) {
  ...
  chapters.forEach((ch, i) => {
    ...
    const baseEmissiveIntensity = 0.10 + ch.accuracy * 0.08;
    ...
    chTags.forEach((tag, j) => {
      ...
      tagSphere.userData = { type: 'tag', id: tag.id, label: tag.label, _baseEmissiveIntensity: 0.06 };
...
function createDotCloud(parentGroup, questions, simMatrix) {
  ...
  const shellRadius = 0.55 + (parentGroup.userData.data.totalQ || 0) / 40;
  const correctColor = new THREE.Color(0.4, 1.0, 0.6);
  const wrongColor = new THREE.Color(1.0, 0.2, 0.1);
...
function createLabelSprite(text, options = {}) {
  ...
  // Subtle shadow for readability (no glow)
...
function startLoop() {
  function animate() {
    ...
    // WASD keyboard pan
    const keys = window._starmapKeys;
    if (keys) {
      const panSpeed = 12 * dt;
      ...
      if (keys.w) panDelta.add(camDir);
      if (keys.s) panDelta.sub(camDir);
      if (keys.a) panDelta.sub(camRight);
      if (keys.d) panDelta.add(camRight);
