...
export async function starmapOnTabActivate(subjectId) {
  ...
  // Override #main constraints to fill full area beyond sidebar+topbar
  const main = document.getElementById('main');
  if (main && !sm._mainOrigStyle) {
    sm._mainOrigStyle = {
      maxWidth: main.style.maxWidth || '',
      padding: main.style.padding || '',
      margin: main.style.margin || '',
  ...
  // Fetch data
  try {
    const res = await fetchWithAuth('/starmap/' + subjectId + '?_=' + Date.now());
    if (!res || !res.ok) {
      showLoading(false);
      showEmpty('无法加载星图数据');
      return;
...
export function starmapOnTabDeactivate() {
  ...
  // Restore #main original styles
  if (sm._mainOrigStyle) {
    const main = document.getElementById('main');
    if (main) {
      main.style.maxWidth = sm._mainOrigStyle.maxWidth;
      main.style.padding = sm._mainOrigStyle.padding;
      main.style.margin = sm._mainOrigStyle.margin;
      main.style.overflow = sm._mainOrigStyle.overflow;
    }
    sm._mainOrigStyle = null;
  }
}
...
/**
 * Generate an exam from selected chapters (called from star map operation panel).
 * Collects all available questions from the given chapters, creates an exam,
 * and starts the quiz.
 */
window.generateExamForChapters = function(chapterIds) {
  ...
  const ignoredQs = st.ignoredQuestions || [];
  const ignoredSet = new Set(ignoredQs);

  function isIgnored(chId, q) {
    const qText = (q.question || '').substring(0, 80);
    const qId = chId + '::' + q.type + '::' + qText;
    return ignoredSet.has(qId);
  }

  chapterIds.forEach(cid => {
    const ch = st.chapters && st.chapters[cid];
    if (!ch) return;

    // Collect from chapter's quizSets (primary source) and flat questions array
    const allChQ = [];
    if (ch.quizSets && ch.quizSets.length > 0) {
      ch.quizSets.forEach(qs => {
        if (qs.questions) qs.questions.forEach(q => allChQ.push(q));
      });
    }
    if (ch.questions) {
      ch.questions.forEach(q => allChQ.push(q));
    }

    // Deduplicate by question text
    const seen = new Set();
    allChQ.forEach((q, idx) => {
      const key = (q.question || '').substring(0, 80);
      if (seen.has(key)) return;
      seen.add(key);
      if (isIgnored(cid, q)) return;
      // Skip already-answered questions (undefined userAnswers means unanswered)
      const ua = ch.userAnswers && ch.userAnswers[idx];
      if (ua !== undefined) return;
      allQuestions.push({ ...q, _srcChapterId: cid });
    });

    // Collect from history (all rounds)
    const history = st.history || [];
    history.filter(r => r.chapterId === cid).forEach(r => {
      if (r.questions) {
  ...
  if (allQuestions.length === 0) {
    alert('所选章节没有可用题目，请先答题积累题库');
    return;
  }

  // Find subject ID from any chapter
  let subjectId = null;
  for (const [sid, subj] of Object.entries(st.subjects || {})) {
    if (subj.chapterIds && subj.chapterIds.some(cid => chapterIds.includes(cid))) {
      subjectId = sid;
      break;
    }
  }
  ...
  const genExam = {
    id: eid,
    name: '星图组卷 ' + new Date().toLocaleString('zh-CN'),
    type: 'exam',
    subjectId: subjectId || '',
  ...
  if (!st.generatedExams) st.generatedExams = {};
  st.generatedExams[eid] = genExam;
  st.currentExamId = eid;

  if (typeof window.saveState === 'function') window.saveState();
  if (typeof window.startExamQuiz === 'function') {
    window.startExamQuiz(eid);
  } else if (typeof window.showScreen === 'function') {
    window.showScreen('quiz');
  }

  alert('✅ 大考卷已生成：共 ' + allQuestions.length + ' 题（' + chapterIds.length + ' 个章节）');
};
...
function setupInteraction() {
  ...
  // Track drag state to suppress hover/click during camera movement
  sm.isDragging = false;
  controls.addEventListener('start', () => { sm.isDragging = true; });
  controls.addEventListener('end', () => { setTimeout(() => { sm.isDragging = false; }, 80); });
...
function onCanvasMouseMove(e) {
  ...
  if (hits.length > 0) {
    const obj = hits[0].object;
    // Walk up to find userData node, then to the parent Group if needed
    let node = obj;
    while (node && (!node.userData || !node.userData.type)) { node = node.parent; }
    // If we stopped at a child mesh, use its parent group for positioning/data
    if (node && node.parent && node.parent.userData && node.parent.userData.type === node.userData.type && node.parent.userData.data) {
      node = node.parent;
    }
...
function handleNodeClick(obj) {
  let node = obj;
  // Walk up to find the topmost node with userData.type (the Group, not the child Mesh)
  while (node && (!node.userData || !node.userData.type)) {
    node = node.parent;
  }
  // If we stopped at a child mesh (no data field), use its parent group
  if (node && node.parent && node.parent.userData && node.parent.userData.type === node.userData.type && node.parent.userData.data) {
    node = node.parent;
  }
...
function applyHover(obj) {
  ...
  // Skip scale expansion for Points (dot clouds) — only scale meshes
  if (obj.scale && !obj.isPoints) {
