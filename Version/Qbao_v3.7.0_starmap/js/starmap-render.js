...
export function initStarmapScene(container) {
  ...
  bloomPass = new UnrealBloomPass(
    ...
    isMobile ? 0.35 : 0.8,  // strength (reduced on mobile)
  ...
  sphereGeoDot = new THREE.SphereGeometry(0.06, 4, 4);
...
export function starmapRenderNodes(data) {
  ...
  chapters.forEach((ch, i) => {
    ...
    const emissiveIntensity = 0.3 + ch.accuracy * 0.4;
    const mat = new THREE.MeshStandardMaterial({
      ...
      transparent: true,
      opacity: 0.75,
      emissive: new THREE.Color(0x335588),
      emissiveIntensity,
    });
    const sphere = new THREE.Mesh(sphereGeoChapter, mat);
    sphere.scale.setScalar(scale);
    sphere.userData = { type: 'chapter', id: ch.id, name: ch.name };
    group.add(sphere);

    // Orbit ring
    const ringGeo = new THREE.TorusGeometry(scale * 1.4, 0.04, 16, 64);
    const ringMat = new THREE.MeshBasicMaterial({ color: 0x5b8fd4, transparent: true, opacity: 0.3 });
    ...
    if (!isMobile) {
      const chLabel = createLabelSprite(ch.name, {
        fontSize: 42, color: '#aaccff', bgAlpha: 0.3,
      });
      chLabel.position.set(0, scale * 2.0, 0);
      group.add(chLabel);
    ...
    chTags.forEach((tag, j) => {
      ...
      tagSphere.userData = { type: 'tag', id: tag.id, label: tag.label };
      tagGroup.add(tagSphere);

      // Tag label sprite (skip on mobile)
      if (!isMobile) {
        const tagLabel = createLabelSprite(tag.label || tag.id, {
          fontSize: 32, color: '#ccccdd', bgAlpha: 0.2,
        });
        tagLabel.position.set(0, tagScale * 2.2, 0);
  ...
  // --- Inter-tag edges (individual Lines for dynamic update) ---
  if (edges && edges.length > 0) {
    interTagLines = [];
    const edgeCap = isMobile ? 100 : 300;
    const validEdges = edges.filter(e => e.weight > 0.15).slice(0, edgeCap);
    for (const e of validEdges) {
      const src = tagGroups[e.source];
      const tgt = tagGroups[e.target];
      if (!src || !tgt) continue;
      ...
      const lineMat = new THREE.LineBasicMaterial({
        color: 0x4facfe,
        transparent: true,
        opacity: Math.min(e.weight * 0.35, 0.4),
      });
      const line = new THREE.Line(lineGeo, lineMat);
      scene.add(line);
      interTagLines.push({ line, sourceId: e.source, targetId: e.target });
    }
  }
...
function tagColorForCategory(cat) {
  switch (cat) {
    case 'error': return 0xe5534b;
    case 'review': return 0x5b8fd4;
    default: return 0x8b9dc3;
  }
}
...
function createDotCloud(parentGroup, questions, simMatrix) {
  ...
  const shellRadius = 1.0 + parentGroup.userData.data.totalQ / 20;
...
function clearNodes() {
  ...
  if (interTagLines && interTagLines.length > 0) {
    interTagLines.forEach(({ line }) => {
      line.material.dispose();
      line.geometry.dispose();
      scene.remove(line);
    });
  ...
  // Sprites were children of chapter/tag groups, already disposed+removed above
  chapterLabelSprites = [];
}

/** Update all tag→chapter and inter-tag connection line positions. Called from UI anim loop. */
export function updateAllConnectionLines(chapterGroups, tagGroups) {
  ...
  // Update inter-tag lines
  if (interTagLines && interTagLines.length > 0) {
    interTagLines.forEach(({ line, sourceId, targetId }) => {
      const src = tagGroups[sourceId];
      const tgt = tagGroups[targetId];
      if (!src || !tgt) return;
      const pos = line.geometry.attributes.position;
