...
// Pure JS, no Three.js dependency. Updates target positions for rendering.

const STIFFNESS = 8.0;
const DAMPING = 4.0;
const CHAPTER_REST_LENGTH = 15.0;
const TAG_ATTRACT_STRENGTH = 0.8;
const TAG_REPULSION_CUTOFF = 2.5;
const CENTER_GRAVITY = 0.02;
const MAX_SPRING_DELTA = 1.5; // clamp max displacement per frame
...
function startForceLoop(chapterGroups, tagGroups, edges) {
  ...
  function step() {
    ...
    applyCooccurrenceForces(tagGroups, edges);
    applyTagRepulsion(tagGroups);

    forceAlpha *= (1 - forceAlphaDecay);

    // Adaptive frequency: slow down as system settles
    const nextMs = forceAlpha > 0.1 ? 33 : forceAlpha > 0.01 ? 66 : 150;
...
/** Tags attracted to their parent chapter */
function applyTagForces(tags, chapters) {
  ...
  Object.values(tags).forEach(tag => {
    ...
    const idealDist = 3.0 + tag.userData.data.totalQ / 20;
...
/** Tags with high co-occurrence attract each other */
function applyCooccurrenceForces(tags, edges) {
  const validEdges = (edges || []).filter(e => e.weight > 0.1);
  for (const e of validEdges) {
    const src = tags[e.source];
    const tgt = tags[e.target];
    if (!src || !tgt) continue;

    const dir = tgt.position.clone().sub(src.position);
    const dist = dir.length();
    if (dist < 0.1) continue;

    const idealDist = 2.0 + (1 - e.weight) * 5.0;
    const diff = dist - idealDist;
    const force = e.weight * Math.abs(diff) * forceAlpha * 0.3;
    dir.normalize().multiplyScalar(force);
    displaceNode(src, dir);
    displaceNode(tgt, dir.clone().multiplyScalar(-1));
  }
}

/** Tags repel each other when too close */
