...
function accuracyToHeatColor(accuracy) {
  ...
  if (accuracy < 0.5) {
    ...
    return new THREE.Color(1, t * 0.8, 0);
  } else if (accuracy < 0.75) {
    const t = (accuracy - 0.5) / 0.25;
    return new THREE.Color(1 - t, 0.8 + t * 0.2, t);
  } else {
    const t = (accuracy - 0.75) / 0.25;
    return new THREE.Color(0, 1 - t * 0.7, 0.3 + t * 0.7);
  }
}
...
function setupInteraction() {
  ...
  if (!sm.raycaster) {
    ...
    sm.raycaster.params.Points.threshold = 0.3;
  }
...
function getInteractables() {
  ...
  chGroups.forEach(g => {
    g.traverse(c => { if (c.isMesh) objects.push(c); });
  });
  Object.values(tagGroups).forEach(g => {
    g.traverse(c => { if (c.isMesh) objects.push(c); });
  });
...
function startUIAnimLoop() {
  ...
  function loop() {
    ...
    // Sprite distance culling: hide labels far from camera
    ...
    if (sprites.length > 0) {
      ...
      sprites.forEach(s => {
        ...
        s.visible = dist < 25;
        // Scale down as distance increases for smooth fade-out feel
        const fade = 1 - Math.max(0, Math.min(1, (dist - 18) / 7));
