...
| `deploy/` | 生产部署资产：systemd 单元、部署手册 |
| `LICENSE` | 上游 MIT 许可原文 |

## 架构（生产）

- 入口 `/games/werewolf/`（Caddy 静态直出 + 子路径 SPA 回退）
- HTTP API `/games/werewolf/api/*` → Caddy `uri strip_prefix` 剥前缀 → `127.0.0.1:3011`
- WebSocket `/games/werewolf/werewolf-ws/*` → `127.0.0.1:3011` 原样透传（**不剥前缀**）
...
## 部署（生产：https://{DOMAIN} 单机直出，Caddy 就地终结 TLS）
...
5. Caddy：`/etc/caddy/Caddyfile` 两个 site 块内各含狼人杀两条路由（API 剥前缀 + WS 不剥），
   改后 `caddy validate --config /etc/caddy/Caddyfile && systemctl reload caddy`（片段见 docs/ARCHITECTURE.md §2.4）
