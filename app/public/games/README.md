...
本目录是 Qbao 的附属休闲站点，由 Vite 原样拷入构建产物（`app/dist/games/`），经 Caddy 静态直出。
...
- 狼人杀为唯一联机游戏：本目录存放其前端构建产物（指向自身房间服务）；后端服务与源码见仓库 `party/werewolf/`（systemd `qbao-werewolf`，端口 3011，Caddy 子路径 `/games/werewolf/`）。
