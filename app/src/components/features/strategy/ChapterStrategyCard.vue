...
<template>
  <div class="card">
    ...
    <div class="strategy-labels">
      <span class="l-err">🔴 针对错题 <input v-model.number="errPct" class="pct-input" type="number" min="0" max="100" @change="onPctInput(0)">%</span>
      <span class="l-review">🟡 滚动复习 <input v-model.number="reviewPct" class="pct-input" type="number" min="0" max="100" @change="onPctInput(1)">%</span>
      <span class="l-new">🟢 新考点 <input v-model.number="newPct" class="pct-input" type="number" min="0" max="100" @change="onPctInput(2)">%</span>
    </div>
    <div class="dual-range-wrap">
      ...
      <input type="range" min="0" max="100" step="1" :value="strategy.errPct" @input="onErrSlider">
      <input type="range" min="0" max="100" step="1" :value="strategy.errPct + strategy.reviewPct" @input="onCumSlider">
    </div>
    <div class="strategy-values">
      <div class="sv-item err"><div class="sv-num">{{ strategy.errPct }}</div><div class="sv-label">🔴 针对错题</div></div>
      <div class="sv-item review"><div class="sv-num">{{ strategy.reviewPct }}</div><div class="sv-label">🟡 滚动复习</div></div>
      <div class="sv-item new"><div class="sv-num">{{ strategy.newPct }}</div><div class="sv-label">🟢 新考点</div></div>
    </div>
...
.sv-label { font-size: var(--fs-xs); color: var(--text-secondary); }
