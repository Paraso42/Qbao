function renderReport() {
  ...
  ch.questions.forEach((q,i)=>{const ua=ch.userAnswers[i];const ci=getCi(q,ua);const tm={single:'单选',judge:'判断',term:'名词解释',short:'简答'};let icon='⏳';if(ua!==undefined)icon=ci===true?'✅':(ci===false?'❌':'✅（主观题）');html+='<div class="history-q-item '+(ci===false?' wrong':ci===true?' correct':'')+'">';html+='<div class="q-text">'+icon+' <span class="q-type-badge">'+(tm[q.type]||q.type)+'</span> 第'+(i+1)+'题：'+renderMarkdown(q.question)+'</div>';if(ua!==undefined){html+='<div class="q-detail">';if(q.type==='single'||q.type==='judge'){var labels=['A','B','C','D','E','F'];var yourLabel=labels[ua]||String(ua);var correctLabel=labels[q.answer]||String(q.answer);var yourMark=ci===true?'✓':'✗';var yaCls=ci===true?'q-answer your-answer correct':'q-answer your-answer wrong';html+='<span class="'+yaCls+'">'+yourMark+' 你的答案：'+yourLabel+'. '+renderMarkdown(String(q.options[ua]??""))+'</span>';html+='<span class="q-answer correct-answer">✓ 标准答案：'+correctLabel+'. '+renderMarkdown(String(q.options[q.answer]??""))+'</span>';}else{html+='<span style="color:var(--text-secondary);font-size:var(--fs-sm);">📝 你的答案：'+escapeHtml(String(ua))+'</span>';}html+='</div>';}if(q.explanation)html+='<div class="q-explanation">📖 '+renderMarkdown(q.explanation)+'</div>';html+='</div>';});
