...
- `server`: `npx vitest run` 全绿（基线 **266 用例 / 43 文件**，2026-09-13 复核；含鉴权媒体票据、迁移完整性 + 路由唯一性、优雅停机、客户端错误上报守卫）
- `server`: 数据库变更后跑 `node scripts/run_migration.js --verify`（打印未落库的迁移编号 / 仓库缺失的已记账版本）
- `app`: `npx vitest run` 全绿（基线 **261 用例 / 27 文件**，2026-09-13 复核）；**登录门禁：未登录整页登录门禁、匿名零写盘、匿名改动锁重建**
