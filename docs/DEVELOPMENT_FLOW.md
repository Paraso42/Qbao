...
- 缓存提醒：生产静态 7 天缓存 —— 改 js/css 时同步更新页面引用版本参数 ?v=，并提醒用户强刷。
  - **受保护媒体（`/api/v1/chat/files/*`、`/api/v1/issues/images/*`）不在此列**：它们由业务端点显式下发 `private, max-age=31536000, immutable`（见 `server/src/lib/mediaCache.js`），反向代理的按扩展名静态缓存规则必须**排除 `/api/*`**。上线前后各测一次响应头，确认 API 响应里只剩一个 `Cache-Control`。
  - Caddy 语法坑：`@name path *.png \` 换行 `not path /api/*` **不是「且」**——`not`/`path`/`/api/*` 会被当成三个扩展名模式塞进同一个 `path` 列表，条件恒假且 `caddy validate` 不报错。要用块式匹配器（`@name { path …; not path /api/* }`）或 `path *.png !/api/*`；改完查 `http://127.0.0.1:2019/config/` 的编译产物确认 `"not"` 是**键**而不是数组元素。
...
- `server`: `npx vitest run` 全绿（基线 **281 例 / 46 文件**，2026-09-16 复核；含鉴权媒体票据、迁移完整性 + 路由唯一性、优雅停机、客户端错误上报守卫、**AI 出题任务全链路 e2e 与自检空转兜底**、**受保护媒体缓存头与两端点防回退**）
- `server`: 数据库变更后跑 `node scripts/run_migration.js --verify`（打印未落库的迁移编号 / 仓库缺失的已记账版本）
- `app`: `npx vitest run` 全绿（基线 **286 用例 / 28 文件**，2026-09-16 复核）；**登录门禁：未登录整页登录门禁、匿名零写盘、匿名改动锁重建**；**图片上传前压缩的安全网（非图片/GIF/小图不压、压大即弃、异常回退原件）**
