...
- `server`: `npx vitest run` 全绿（基线 **275 例（270 通过 / 5 跳过）/ 45 文件**，2026-09-16 复核；含鉴权媒体票据、迁移完整性 + 路由唯一性、优雅停机、客户端错误上报守卫、**AI 出题任务全链路 e2e 与自检空转兜底**）
- `server`: 数据库变更后跑 `node scripts/run_migration.js --verify`（打印未落库的迁移编号 / 仓库缺失的已记账版本）
- `app`: `npx vitest run` 全绿（基线 **264 用例 / 27 文件**，2026-09-16 复核）；**登录门禁：未登录整页登录门禁、匿名零写盘、匿名改动锁重建**
