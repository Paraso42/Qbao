...
- `server`: `npx vitest run` 全绿（基线 **312 例 / 49 文件**，2026-09-17 复核；含鉴权媒体票据、迁移完整性 + 路由唯一性、优雅停机、客户端错误上报守卫、**AI 出题任务全链路 e2e 与自检空转兜底**、**受保护媒体缓存头与两端点防回退**、**列表缩略图 ?w= 与票据时间桶稳定性**、**聊天媒体四道闸门（配额/水位/归属/回收）与下载侧次数+字节双预算**）
