...
### 3.4 混合产物（桌面安装包 / 手机壳 App / 网页构建产物）如何适用

实际分发物是**混合体**：Qbao 自研代码 + 上述第三方组件打包在一起。适用规则：

- **Qbao 自研部分**仍然禁止商业使用——把整个安装包/站点用于商业目的，不因其中含 MIT 组件而获得豁免；
- 第三方组件部分（游戏、npm 依赖）**保留其原有许可**，作者不会、也无权对这些部分附加非商业限制；
- 因此若有人只想复用其中的 MIT 游戏，应从**其上游仓库**取用，而非从本项目转手（弹猪乐除外，见 §3.2）。
