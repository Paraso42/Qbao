...
**安装版（打包后）**：首次启动会弹出「设置服务器地址」引导框，输入服务器地址（如 `https://your-server.example`，自动补全 `/api/v1`）后窗口自动重建并连接。地址保存在用户数据目录 `settings.json`，后续可在应用内修改。**apiBase 留空 = 纯本地/离线模式**（账号与 AI 出题不可用，本地题库与答题可用）。
